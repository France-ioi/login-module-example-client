...
function run($action) {
    ...
    try {
        switch($action) {
            case 'create':
                $res = $manager->create($_POST['prefix'], $_POST['amount']);
                break;
            case 'delete':
...
<html>
    ...
    <body>
        ...
        <form action="<?=$_SERVER['PHP_SELF']?>" method="POST">
            ...
            <div>
                Prefix <input type="text" name="prefix" value="<?=isset($_POST['prefix'])?$_POST['prefix']:'test'?>"/>
            </div>
            <div>
                Amount <input type="text" name="amount" value="<?=isset($_POST['amount'])?$_POST['amount']:'1'?>"/>
            </div>
            <input type="submit"/>
        </form>
